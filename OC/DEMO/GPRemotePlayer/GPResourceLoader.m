...
@interface GPResourceLoader ()<GPAudioDownLoaderDelegate>

@property (nonatomic, strong) GPAudioDownLoader *downLoader;
...
@implementation GPResourceLoader

- (GPAudioDownLoader *)downLoader {
    if(!_downLoader) {
        _downLoader = [[GPAudioDownLoader alloc] init];
...
- (void)handleAllRequest {
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        NSData *data = [NSData dataWithContentsOfFile:[GPAudioFileTool tmpPathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data.length == 0) {
            data = [NSData dataWithContentsOfFile:[GPAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if ([GPAudioFileTool isCacheFileExists:url])
    {
        // 三个步骤, 直接响应数据
...
- (void)handleRequestWithLoadingRequest: (AVAssetResourceLoadingRequest *)loadingRequest {
    NSURL *url = [loadingRequest.request.URL httpURL];
    // 1. 填充信息头
    loadingRequest.contentInformationRequest.contentType = [GPAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = [GPAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    
    // 2. 响应数据
    NSData *data = [NSData dataWithContentsOfFile:[GPAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
