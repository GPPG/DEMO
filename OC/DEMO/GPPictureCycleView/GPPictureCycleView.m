...
#import "GPPictureCycleView.h"

static NSInteger const radio = 10;

@interface GPPictureCycleView()<UIScrollViewDelegate>
{
...
@implementation GPPictureCycleView

#pragma mark - init
+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {

    GPPictureCycleView *adView = [[GPPictureCycleView alloc] init];
...
-(void)setPicModels:(NSArray<id <GPPictureCycleProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<GPPictureCycleProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<GPPictureCycleProtocol> adM = self.picModels[tag];
