...
@interface GPDownLoader ()<NSURLSessionDataDelegate>
{
    // 临时文件的大小
...
+ (NSString *)downLoadedFileWithURL: (NSURL *)url {
    ...
    if([GPDownLoaderFileTool isFileExists:cacheFilePath]) {
...
- (void)downLoadWithURL: (NSURL *)url {
    ...
    // 文件的位置, 文件的大小
    if ([GPDownLoaderFileTool isFileExists:self.cacheFilePath]) {
        NSLog(@"文件已经下载完毕, 直接返回相应的数据--文件的具体路径, 文件的大小");
        
        if (self.downLoadInfo) {
            self.downLoadInfo([GPDownLoaderFileTool fileSizeWithPath:self.cacheFilePath]);
        }
        
        self.state = GPDownLoaderStateSuccess;
    ...
    if ([url isEqual:self.task.originalRequest.URL]) {
        // 任务存在 -> 状态
        // 状态 -> 正在下载 返回
        if (self.state == GPDownLoaderStateDowning)
        {
            return;
        }
        // 状态 -> 暂停 = 恢复
        if (self.state == GPDownLoaderStatePause)
        {
            [self resume];
            return;
        }
    ...
    [self cancel];
    // 2. 读取本地的缓存大小
    _tmpFileSize = [GPDownLoaderFileTool fileSizeWithPath:self.tmpFilePath];
...
- (void)resume {
    if (self.state == GPDownLoaderStatePause) {
       [self.task resume];
        self.state = GPDownLoaderStateDowning;
    }
}
...
- (void)pause {
     ...
     {
        [self.task suspend];
         self.state = GPDownLoaderStatePause;
     }
...
- (void)cancelAndClearCache {
    ...
    // 删除缓存
    [GPDownLoaderFileTool removeFileAtPath:self.tmpFilePath];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    if (_tmpFileSize == _totalFileSize) {
        NSLog(@"文件已经下载完成, 移动数据");
        // 移动临时缓存的文件 -> 下载完成的路径
        [GPDownLoaderFileTool moveFile:self.tmpFilePath toPath:self.cacheFilePath];
        self.state = GPDownLoaderStateSuccess;
        // 取消请求
...
//        error.localizedDescription;
        self.state = GPDownLoaderStateFailed;
...
- (void)setState:(GPDownLoaderState)state {
    if (_state == state) {
        return;
    }
    _state = state;
