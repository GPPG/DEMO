...
#import "GPSqliteModelTool.h"
#import "GPSqliteTool.h"
#import "GPModelTool.h"
#import "GPTableTool.h"


@implementation GPSqliteModelTool
...
+ (BOOL)createTableWithModelClass: (Class)cls withUID: (NSString *)uid {
    ...
    // 1. 判断 表是否存在
    NSString *tableName = [GPModelTool getTableNameWithModelClass:cls];
    BOOL isTableExists = [GPTableTool isTableExists:tableName uid:uid];
    ...
    NSDictionary *ivarNameTypeDic = [GPModelTool getModelIvarNameSqlTypeDic:cls];
    ...
    if (!isTableExists) {
        ...
        return [GPSqliteTool dealSql:createTabelSQL withUID:uid];
    }


    // 表需要更新
    NSString *tmpTableName = [GPModelTool getTempTableNameWithModelClass:cls];
    ...
    NSArray *columnNames = [GPTableTool getTableAllColumnNames:tableName uid:uid];
    NSArray *modelColumnNames = [GPModelTool getModelIvarNames:cls];
    ...
    return [GPSqliteTool dealSqls:updateSqls withUID:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *rowDicArray = [GPSqliteTool querySql:sql withUID:uid];
...
+ (NSArray *)queryModels:(Class)cls sql: (NSString *)sql uid: (NSString *)uid {
    ...
    NSArray *array = [GPSqliteTool querySql:sql withUID:uid];
...
+ (BOOL)deleteModel: (id)model uid: (NSString *)uid {
    NSString *tableName = [GPModelTool getTableNameWithModelClass:[model class]];
    ...
    return [GPSqliteTool dealSql:sql withUID:uid];
...
+ (BOOL)saveModel: (id)model uid: (NSString *)uid {
    ...
    NSArray *res = [GPSqliteTool querySql:sql withUID:uid];

    // 字段
    NSArray *columnNames = [GPModelTool getModelIvarNames:[model class]];
    ...
    if (res.count > 0) {
        ...
        return [GPSqliteTool dealSql:insertSql withUID:uid];
    }
...
+ (NSArray *)parseRowDicArray: (NSArray *)rowDicArray withModelClass: (Class)cls {
    ...
    // 模型真正的字段 - > 类型
    NSDictionary *modelColumnNames = [GPModelTool getModelIvarNameIvarTypeDic:cls];
...
+ (BOOL)isRequireUpdate:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *tableColumnNames = [GPTableTool getTableAllColumnNames:[GPModelTool getTableNameWithModelClass:cls] uid:uid];
...
+ (NSDictionary *)naoTypeSQLRelation {
    ...
    return @{
             @(GPSqliteModelToolNAONot) : @"not",
             @(GPSqliteModelToolNAOAnd) : @"and",
             @(GPSqliteModelToolNAOOr) : @"or"
             };
